...
struct Bus: sc_module
{
  ...
  {
    if (static_cast<unsigned int>(id)< N_INITIATORS)
    {
      // Forward path
  ...
  inline unsigned int decode_address( sc_dt::uint64 address, sc_dt::uint64& masked_address )
  {
    ...
    if(address>=ADDER_INPUT_ADDR_BASE&&address<(ADDER_INPUT_ADDR_BASE+ADDER_INPUT_ADDR_SIZE)){
      masked_address = address-ADDER_INPUT_ADDR_BASE;
      target_nr=0;
    }
    else if(address>=ADDER_OUTPUT_ADDR_BASE&&address<(ADDER_OUTPUT_ADDR_BASE+ADDER_OUTPUT_ADDR_SIZE)){
      masked_address = address-ADDER_OUTPUT_ADDR_BASE;
      target_nr=0;
    }
    else if(address>=BUFFER_ADDR_BASE&&address<(BUFFER_ADDR_BASE+BUFFER_ADDR_SIZE)){
      masked_address = address-BUFFER_ADDR_BASE;
      target_nr=1;
    }
    else{
      SC_REPORT_ERROR("TLM-2", "Address range not supported by bus.");
    }
    return target_nr;
  }
