...
SC_MODULE(Adder)
{
  ...
  virtual void b_transport( tlm::tlm_generic_payload& trans, sc_time& delay )
  {
    ...
    if ( cmd == tlm::TLM_READ_COMMAND ){
      ...
      switch(adr){
        case ADDER_INPUT_R1:
          for (unsigned int i = 0; i < len; i++) i_data[0].range(3+(i<<2), i<<2) = ptr[i];
          delay=sc_time(5, SC_NS); 
          break;
        case ADDER_INPUT_R2:
          for (unsigned int i = 0; i < len; i++) i_data[1].range(3+(i<<2), i<<2) = ptr[i];
          //寫入讀兩個，有一個會重複使用(temp)所以存起來，然後成以權重累加
          for (unsigned int i = 0; i < len; i++) i_data[i] = ptr[i];
          sum=i_data[1]/2+i_data[0]/3+i_temp/6;
          i_temp = i_data[1];
