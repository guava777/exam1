...
SC_MODULE(stim) {
  ...
  rdyvld_port_out<sc_ufixed_fast<4, 1> > o_a;
  //o_b output ports: message, ready, valid signals
  rdyvld_port_out<sc_ufixed_fast<4, 1> > o_b;
  //i_sum input ports: message, ready, valid signals
  rdyvld_port_in<sc_ufixed_fast<4, 1> > i_sum;
  //Store the previous inputs to FIFOs
  sc_ufixed_fast<4, 1> t_a;
  sc_uint<1> t_a_rdy;
  sc_uint<1> t_a_vld;
  sc_ufixed_fast<4, 1> t_b;
  sc_uint<1> t_b_rdy;
  sc_uint<1> t_b_vld;
